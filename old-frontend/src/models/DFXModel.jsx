...
import {getDxf} from "../services/dxf";
...
const DXFModel = () => {
    ...
    const [token, setToken] = useState('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOi8vMTI3LjAuMC4xOjgwMDAvYXBpL2xvZ2luIiwiaWF0IjoxNzQzMTU5MDUxLCJleHAiOjE3NDMxNjI2NTEsIm5iZiI6MTc0MzE1OTA1MSwianRpIjoiVnFuRmFwNFo0Z0NIeUVxOCIsInN1YiI6IjEiLCJwcnYiOiIyM2JkNWM4OTQ5ZjYwMGFkYjM5ZTcwMWM0MDA4NzJkYjdhNTk3NmY3Iiwicm9sZSI6bnVsbH0.t-kjz452l7kk3kN66WRGV-iE4Vl4VgDWY9IZNT-Wmhc'); // Get this token from Laravel Breeze after login
    useEffect(() => {
        const fetchDxf = async () => {
            try {
                const dxfData = await getDxf(token);
                setDxf(dxfData.path);
            } catch (error) {
    ...
    useEffect(() => {
        fetch('http://127.0.0.1:8000/api/files/dxf-files/BwXYdeqq7dDUCAajWibgzggsJVIZbswG4jyjIH2O.txt',{
            headers: {
                ...
                'Authorization': `Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOi8vMTI3LjAuMC4xOjgwMDAvYXBpL2xvZ2luIiwiaWF0IjoxNzQzMTU5MDUxLCJleHAiOjE3NDMxNjI2NTEsIm5iZiI6MTc0MzE1OTA1MSwianRpIjoiVnFuRmFwNFo0Z0NIeUVxOCIsInN1YiI6IjEiLCJwcnYiOiIyM2JkNWM4OTQ5ZjYwMGFkYjM5ZTcwMWM0MDA4NzJkYjdhNTk3NmY3Iiwicm9sZSI6bnVsbH0.t-kjz452l7kk3kN66WRGV-iE4Vl4VgDWY9IZNT-Wmhc` // Add this line
            }
        })
            .then(response => {
...
const Entity = ({ entity }) => {
    ...
    const { scene, nodes, materials } = useGLTF('/foxs_islands.glb');
    ...
    switch (entity.type) {
        ...
        case 'POLYLINE':
            return entity.vertices ? (
                <group >
                    ...
                    {/* Example of using GLTF nodes and materials */}
                    {nodes && materials && (
                        <>
                            <mesh
                                castShadow
        ...
        /> */}
                            ...
                            {/*<Door rotation={new THREE.Euler(Math.PI  ,Math.PI/1.9 ,Math.PI)}*/}
                            {/*      position={[2.6, 0, 5.5]}*/}
                            {/*/>*/}
                        </>
                    )}
